...
'KG':{title:'Growing capable learners.',copy:'Children strengthen communication, early numeracy, creativity and confidence through purposeful learning experiences.',img:assetPath+'ais-early-years-enhanced-4k.jpg'},
'PRIMARY':{title:'Knowledge with purpose.',copy:'Learners build academic strength while developing critical thinking, responsibility, collaboration and confidence.',img:assetPath+'ais-documentary-classroom-enhanced-4k.jpg'},
...
function setStage(el){const d=stageData[el.dataset.stage];if(!d||!stageImg)return;stages.forEach(x=>x.classList.remove('active'));el.classList.add('active');stageImg.style.opacity='0';setTimeout(()=>{stageImg.src=d.img;stageImg.alt='AIS students — '+el.dataset.stage.toLowerCase();stageTitle.textContent=d.title;stageCopy.textContent=d.copy;stageLabel.textContent=el.dataset.stage;stageImg.style.opacity='1'},160)}
stages.forEach(s=>s.addEventListener('click',()=>setStage(s)));

const facilityData={
CLASSROOMS:['Learning spaces designed for focus, interaction and purposeful learning.',assetPath+'ais-classroom-wide-enhanced-4k.jpg'],
SCIENCE:['Science and laboratory imagery will be added from verified AIS photographs.',null],
ICT:['ICT learning imagery will be added from verified AIS photographs.',null],
LIBRARY:['Library imagery will be added from verified AIS photographs.',null],
SPORTS:['Outdoor activity and sports imagery from AIS documentary footage.',assetPath+'ais-sports-field-enhanced-4k.jpg'],
CAMPUS:['The AIS campus at Bustan-e-Ahmad, Ashongman.',assetPath+'ais-campus-documentary-enhanced-4k.jpg']};
...
function setFacility(t){const d=facilityData[t.dataset.facility];if(!d||!fi)return;ft.forEach(x=>x.classList.remove('active'));t.classList.add('active');const parent=fi.parentElement;fi.style.opacity='0';setTimeout(()=>{parent.querySelector('.facility-placeholder')?.remove();if(d[1]){fi.style.display='block';fi.src=d[1]}else{fi.style.display='none';const ph=document.createElement('div');ph.className='facility-placeholder';ph.innerHTML=`<div><span>Verified photography pending</span><strong>${t.dataset.facility}</strong><p>${d[0]}</p></div>`;parent.insertBefore(ph,parent.firstChild)}fh.textContent=t.dataset.facility;fp.textContent=d[0];fi.style.opacity='1'},160)}
...
items.forEach(i=>i.addEventListener('click',()=>{if(!lb)return;const img=i.querySelector('img');if(!img)return;lbImg.src=img.src;lb.classList.add('open');document.body.style.overflow='hidden'}));
