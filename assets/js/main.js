...
const stageData={
'Early Years':null,
'EARLY YEARS':{title:'A confident beginning.',copy:'A nurturing environment where children discover the joy of learning, build independence and develop strong social foundations.',img:assetPath+'ais-campus-left-clean.jpg'},
'NURSERY':{title:'Curiosity starts here.',copy:'Play, language, exploration and relationships come together to create a strong foundation for lifelong learning.',img:assetPath+'ais-campus-center-clean.jpg'},
'KG':{title:'Growing capable learners.',copy:'Children strengthen communication, early numeracy, creativity and confidence through purposeful learning experiences.',img:assetPath+'ais-documentary-thumb.jpg'},
'PRIMARY':{title:'Knowledge with purpose.',copy:'Learners build academic strength while developing critical thinking, responsibility, collaboration and confidence.',img:assetPath+'ais-campus-left-clean.jpg'},
'JUNIOR HIGH SCHOOL':{title:'Ready for the next chapter.',copy:'Students deepen subject knowledge, leadership, discipline and independent thinking as they prepare for future pathways.',img:assetPath+'ais-campus-grounds-clean.jpg'}};
...
const facilityData={CLASSROOMS:['Learning spaces designed for focus and interaction.',assetPath+'ais-campus-center-clean.jpg'],SCIENCE:['The science environment should be populated with approved AIS photography before launch.',assetPath+'ais-campus-center-clean.jpg'],ICT:['The ICT environment should be populated with approved AIS photography before launch.',assetPath+'ais-campus-left-clean.jpg'],LIBRARY:['A place for reading, research, reflection and discovery.',assetPath+'ais-documentary-thumb.jpg'],SPORTS:['Sports imagery should be added from the approved AIS archive.',assetPath+'ais-campus-grounds-clean.jpg'],CAMPUS:['The AIS campus at Bustan-e-Ahmad, Ashongman.',assetPath+'ais-campus-hero-clean.jpg']};
...
// Premium documentary modal: keeps the homepage clean and loads YouTube only when requested.
(function(){const modal=document.querySelector('.video-modal');if(!modal)return;const frame=modal.querySelector('iframe');const close=()=>{modal.classList.remove('is-open');modal.setAttribute('aria-hidden','true');frame.src='about:blank';document.body.classList.remove('modal-open')};document.querySelectorAll('.video-open').forEach(b=>b.addEventListener('click',()=>{frame.src=b.dataset.video+'&autoplay=1';modal.classList.add('is-open');modal.setAttribute('aria-hidden','false');document.body.classList.add('modal-open')}));modal.querySelector('.video-modal-backdrop').addEventListener('click',close);modal.querySelector('.video-modal-close').addEventListener('click',close);document.addEventListener('keydown',e=>{if(e.key==='Escape')close()})})();
